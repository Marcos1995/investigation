...
- Ranking: Atenas, Roma, Varsovia, Lisboa, Palma, París, Londres, Milán, Madrid, Viena
...
2. **Filtros de vuelo:** ida y vuelta, solo **directos**, 2 adultos, cabina ~8 kg, **tarde ida y tarde vuelta** (~15:00). Si no hay vuelta de tarde, la más tardía (no emparejar con las 06:00).
...
6. **Airbnb:** 6 adultos, piso entero, 3 hab. + 2 baños, mapa del centro, 4–8 dic. Total de estancia (no precio/noche suelto). Ranking usa este total. Si un mínimo es la mitad del resto del cluster (p. ej. Londres 786 vs 1434), usa el suelo del cluster.
7. **Hotel:** 3 habitaciones con baño × 4 noches = nightly × 12. Sin hostales. Se muestra aparte, no decide el ranking.
8. **Comida:** 40 € × 4 días × 6 = 960 (el slider de la web).
9. **Total grupo** = 2×(BCN+FRA+HAM pp, ya con cabina y asientos) + Airbnb + 960. Recalcula `rank` y el texto `why`.
10. **Hora** en el pie (es/en/de) y esta línea de ranking. Si una ciudad se dispara y hay otra con tarde desde los **tres** orígenes al **mismo** IATA y más barata, sustitúyela.

No olvidar ningún coste: ni vuelo, ni cabina, ni asientos juntos, ni Airbnb, ni hotel. Si no está en el checklist, no está en el número.
